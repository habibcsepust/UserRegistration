...
SELECT top 10 CONCAT(fName,+' '+lName) as name,tuser.phoneNo as phone, tuser.emailNo as email,tuser.userImg as Image, tcity.cityName as city, tcountry.countryName as country from TblUsers tuser
	left join TblCity tcity ON tuser.userCity = tcity.Id
	left join TblCountry tcountry ON tc.CountryId = tcountry.Id where tu.emailNo = 'admin@admin.com'
